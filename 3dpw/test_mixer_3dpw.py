        ...
        if a<8:
            ...
            with torch.no_grad():
                ...
                if args.delta_x:
                    ...
                    sequences_train_delta = sequences_train_delta.reshape(bs, 10, 23, -1).permute(0, 3, 2, 1)
                    ...
                    sequences_predict_3d = sequences_predict.reshape(sequences_predict.shape[0], sequences_predict.shape[1], -1,
                                                                     3)
                    ...
                    for bat in range(predicted_3d_pos.shape[0]):
                        if bat != 112:
                            ...
                            # bat = bat + 24*bat
                            if os.path.exists('/home/young/Lab/Code/Prediction/MotionMixer-main_3DPW/3dpw/aaafig{}'.format(bat)) == False:
                                os.mkdir('/home/young/Lab/Code/Prediction/MotionMixer-main_3DPW/3dpw/aaafig{}'.format(bat))
                            print('frame:', bat)
                            af=predicted_3d_pos.shape[1]
                            for f in range(0,af):
                                points = predicted_3d_pos[bat, f, :, :].tolist()
                                points2 = gt_3d[bat, f, :, :].tolist()
                                # points2 = points
                                conect_point2 = [[0, 2], [1, 2], [0, 3], [3, 6], [6, 9], [1, 4], [4, 7], [7, 10], [2,5], [5, 8],
                                 [8, 12],
                                ...
                                for connect in conect_point2:
                                    ...
                                    ax2.plot3D([points2[connect[0]][0], points2[connect[1]][0]],
                                ...
                                fig.savefig(os.path.normpath(os.path.join
                                                         ('/home/young/Lab/Code/Prediction/MotionMixer-main_3DPW/3dpw/aaafig{}'.format(bat), 'pre_{}_{}.jpg'.format(bat,f))), dpi=1000)
                                fig2.savefig(os.path.normpath(os.path.join
                                                             ('/home/young/Lab/Code/Prediction/MotionMixer-main_3DPW/3dpw/aaafig{}'.format(
                                                                 bat), 'gt_{}_{}.jpg'.format(bat, f))), dpi=1000)
                ...
                loss = mpjpe_error(all_joints_seq[:, 0:args.output_n, :].view(-1, args.output_n, 24, 3),
                                   all_joints_seq_gt[:, 0:args.output_n, :].view(-1, args.output_n, 24, 3))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default='./checkpoints/frame{}_block3_notg.pt'.format(output_n),
    ...
    if args.loss_type == 'mpjpe':
        ...
        parser_mpjpe.add_argument('--num_blocks', default=3, type=int, required=False)
